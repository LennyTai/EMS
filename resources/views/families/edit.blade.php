...
@endif
{{ Form::model($family, ['method' => 'PATCH', 'route' => ['families.update', $family->id], 'enctype' => 'multipart/form-data']) }}
@csrf
<div class="row">
    <div class="col-sm-12">
        <div class="card">
            ...
            <div class="row">
                ...
                <div class="form-group col-sm-3">
                    ...
                    {{ Form::text('family_name', $family->family_name, ['class' => 'form-control']) }}
                </div>
               <div class="form-group col-sm-3">
                    {{ Form::label('relationship', 'Relationship', ['class' => 'required']) }}
                    {{ Form::select('relationship', __('message.relationship'), $family->relationship, ['class' => 'form-control', 'placeholder' => '[Please Select]' ]) }}
                </div>
                <div class="form-group col-sm-3">
                    {{ Form::label('family_dob', 'Date of Birth', ['class' => 'required']) }}
                    {{ Form::date('family_dob', $family->family_dob, ['class' => 'form-control', 'max' => '2999-12-31']) }}
                </div>
                <div class="form-group col-sm-3">
                    {{ Form::label('family_contact', 'Contact Number', ['class' => 'required']) }}
                    {{ Form::number('family_contact', $family->family_contact, ['class' => 'form-control']) }}
                </div>
            </div>
            <hr>
            <div class="row">
            ...
            {{ link_to(URL::route('families.index', $emp_id), 'Back', ['class' => 'btn btn-primary', 'style' => 'float: right']) }}
            {{ Form::close() }}
            </div>
        </div>
    </div>
